...
)
{
	...
	//continue until the list is empty or the budget > total_cost
	while(!todo->empty() && budget <= total_cost)
	{	//max is the best item found at the time, reset once added to the results
		max=0;
		for(int i=0; i < todo->size(); i++)
		{	//compare the current max and next value by defense/cost
			next = (todo->at(i)->defense() / todo->at(i)->cost());
			if(max < next)
				{
					index=i;
					max=next;
		...
		//check if the potential added budget is within constraints
		//if within the restraints, add the item to the result list
		//add the item's cost to the budget
		//delete the current best item to prevent repeating the same item
			g = todo->at(index)->cost();
			if((g + budget) <= total_cost)
			{
				result->push_back(todo->at(index));
				budget += todo->at(index)->cost();
			}
